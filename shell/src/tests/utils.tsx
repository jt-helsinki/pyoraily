...
export const userSessionSetup = (): void => {
  UserSessionStateManager.resetToNew();
  jest.spyOn(UserServerStateManager, 'fetchUserRoles').mockImplementation(jest.fn());
  const fetchUserRolesQuery = jest.spyOn(ServerState, 'fetchQuery');
  fetchUserRolesQuery.mockImplementation(() => Promise.resolve(MOCK_USER_ROLES));
  LoginService.signIn(mockFetchProfileResponseDate);
};
